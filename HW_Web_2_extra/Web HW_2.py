...
class Phone(Field):
    ...
    def value(self, phone):
        print(f"WTF phone verification {self, phone, type(phone)}")
...
#         print(f"WTF phone verification {phone, type(phone)}")
#         if phone.isdigit() and len(phone) == 10:
#             return True
#         else:
#             raise VerificationError
...
class Email(Field):
    ...
    def value(self, email):
        print(f"WTF phone verification {self, email, type(email)}")
        regex = re.compile(
...
class Birthday(Field):
    ...
    def value(self, b_day: str) -> date:
        ...
        b_day_date = datetime.strptime(b_day, '%d/%m/%Y').date()  # '%Y/%m/%d'
...
class AddressBook(UserDict):
    ...
    def search_contacts(self, name):
        # print(f"@@@@@@@{self.data.items()}")
        search_list = []
        for key, value in self.data.items():
            if name in key:
                #print(f"!! i am in key search {self.get_contact(key)}")
                search_list.append(self.get_contact(key))
            elif name in value.get_info():
                #print(f"?? i am in value search {self.get_contact(key)}")
                search_list.append(self.get_contact(key))

        if len(search_list) > 0:
            # header = "I found: "
            # res = (item for item in search_list)
            # return header + "\n" + next(res) + "\n" + next(res)
...
class Record(Field):
    ...
    def get_info(self):
        ...
        if self.notes:
            ...
            #notes_info = f'Notes: {[notes for notes in self.notes]}'
            #notes_info = f'Notes: {", ".join([notes.value for notes in self.notes])}'
        if self.address:
            address_info = f'Lives: {self.address.value.capitalize()}'
        result = b_day_info, email_info, notes_info, address_info, phones_info
