...
class GetBDays:
    def __init__(self, timedelta, users):
        self.data = users.data.values()
        self.list_bdays = []
        for item in self.data:
            # print(item.day_to_b_day())
            if item.day_to_b_day() and item.day_to_b_day() <= int(timedelta):
                self.list_bdays.append((item.day_to_b_day(), item.name))
        self.list_bdays.sort(key=lambda x: (x[0]))

    def get_info(self):
        # print(self.list_bdays)
...
def manual():
    ...
    >>edit 'name' 'attribute (one of: phones, notes, b_day, email, address)' 'old_value, if not defined = 0' 'new_value', for notes: 'hashtag' 'notes',
...
def birthday_list(timedelta, users=users):
    res = GetBDays(timedelta, users).get_info()
    return {k: v.value.capitalize() for k, v in res}
